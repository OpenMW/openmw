...
namespace ESMTerrain
{
    ...
    void Storage::fixColour (osg::Vec4ub& color, int cellX, int cellY, int col, int row, LandCache& cache)
    {
        ...
        if (data)
        {
            color[0] = data->mColours[col*ESM::Land::LAND_SIZE*3+row*3];
            color[1] = data->mColours[col*ESM::Land::LAND_SIZE*3+row*3+1];
            color[2] = data->mColours[col*ESM::Land::LAND_SIZE*3+row*3+2];
        }
        else
        {
            color[0] = 255;
        ...
        for (int y=0; y<blendmapSize; ++y)
        {
            for (int x=0; x<blendmapSize; ++x)
            {
                ...
                if (found == textureIndicesMap.end())
                {
                    ...
                    // look for duplicate which may happen when two plugins use the same texture
                    ...
                    found = textureIndicesMap.insert(std::make_pair(id, layerIndex)).first;
