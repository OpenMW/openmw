...
class GenericObjectCache : public osg::Referenced
{
    public:
        ...
        void addEntryToObjectCache(const KeyType& filename, osg::Object* object, double timestamp = 0.0)
        {
            OpenThreads::ScopedLock<OpenThreads::Mutex> lock(_objectCacheMutex);
        ...
        void removeFromObjectCache(const KeyType& fileName)
        {
            OpenThreads::ScopedLock<OpenThreads::Mutex> lock(_objectCacheMutex);
            typename ObjectCacheMap::iterator itr = _objectCache.find(fileName);
            if (itr!=_objectCache.end()) _objectCache.erase(itr);
        }

        /** Get an ref_ptr<Object> from the object cache*/
        osg::ref_ptr<osg::Object> getRefFromObjectCache(const KeyType& fileName)
        {
            OpenThreads::ScopedLock<OpenThreads::Mutex> lock(_objectCacheMutex);
            typename ObjectCacheMap::iterator itr = _objectCache.find(fileName);
        ...
        bool checkInObjectCache(const KeyType& fileName, double timeStamp)
        {
            OpenThreads::ScopedLock<OpenThreads::Mutex> lock(_objectCacheMutex);
            typename ObjectCacheMap::iterator itr = _objectCache.find(fileName);
            if (itr!=_objectCache.end())
