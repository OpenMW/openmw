...
namespace MWRender
{
    ...
    void RenderingManager::resetShadowSettings(){
        ...
        if (enableOQN && Settings::Manager::getBool("shadow cams OQN enable", "OcclusionQueries"))
            shadowCastingTraversalMask |= SceneUtil::Mask_OcclusionQuery;
        ...
        shadowManager->getShadowTechnique()->setSceneMask(SceneUtil::Mask_Scene | SceneUtil::Mask_Lighting | SceneUtil::Mask_Actor| SceneUtil::Mask_ParticleSystem);
        unsigned int computefarmask = SceneUtil::Mask_Static | SceneUtil::Mask_Object;

        if (Settings::Manager::getBool("include terrain in far plane computation", "Shadows"))
            computefarmask |= SceneUtil::Mask_Terrain;// doesn't narrow enough far plane for distant terrain

        shadowManager->getShadowTechnique()->setComputeFarMask(computefarmask);

        if(enableOQN)
        {
            shadowManager->getShadowTechnique()->setOcclusionQueryMask(SceneUtil::Mask_OcclusionQuery);
        }
        else
        {
        ...
        , mBorders(false)
    {
        ...
        if (mDistantTerrain)
        {
            ...
            mTerrain.reset(new Terrain::QuadTreeWorld(sceneRoot, mRootNode, mResourceSystem, mTerrainStorage,
                               compMapResolution, compMapLevel, lodFactor, vertexLodMod, maxCompGeometrySize,
                               mObjects->getOcclusionSettings()));
        }
        else
            mTerrain.reset(new Terrain::TerrainGrid(sceneRoot, mRootNode, mResourceSystem, mTerrainStorage, mObjects->getOcclusionSettings()));
