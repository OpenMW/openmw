...
public:
    ...
    osg::StateSet* getShadowedStateSet() {if(_PSSMShadowSplitTextureMap.empty())init();return  _PSSMShadowSplitTextureMap[0]._stateset.get();}

    /** set Shadow Camera cull mode */
    void setCullingMode(bool b) {
        _enableCulling=b;
    }
    ...
    inline void setSplitCount(unsigned int i) {
        _number_of_splits = i;
    }

    /** Get the texture resolution */
    inline unsigned int getSplitCount() const {
        return _number_of_splits;
    }

    /** Set shadow textures offset */
    inline void setTextureOffset(unsigned int i) {
        _textureUnitOffset = i;
    }

    /** Get shadow textures offset */
    inline unsigned int getTextureOffset() const {
        return _textureUnitOffset;
    }
    ...
    inline void setDebugColorOn() {
        _debug_color_in_GLSL = true;
    }

    /** Set the polygon offset osg::Vec2f(factor,unit) */
    inline void setPolygonOffset(const osg::Vec2f& p) {
        _polgyonOffset = p;
        _user_polgyonOffset_set=true;
    }

    /** Get the polygon offset osg::Vec2f(factor,unit) */
    inline const osg::Vec2f& getPolygonOffset() const {
        return _polgyonOffset;
    }

    /** Set the texture resolution */
    inline void setTextureResolution(unsigned int resolution) {
        _resolution = resolution;
    }

    /** Get the texture resolution */
    inline unsigned int getTextureResolution() const {
        return _resolution;
    }
     ...
     */
    inline void setUserLight(osg::Light* light) {
        _userLight = light;
    }

    /** get the user defined light for shadow simulation */
    inline const osg::Light* getUserLight() const {
    ...
    inline void enableShadowGLSLFiltering(bool filtering = true) {
    ...
    inline void setSplitCalculationMode(SplitCalcMode scm=SPLIT_EXP) {
        _SplitCalcMode = scm;
    }

    /** get split calculation mode */
    inline SplitCalcMode getSplitCalculationMode() const {
