...
SkyManager::SkyManager(osg::Group* parentNode, Resource::SceneManager* sceneManager)
    ...
    , mWeatherAlpha(0.f)
{
    osg::ref_ptr<CameraRelativeTransform> skygroup (new CameraRelativeTransform);
    osg::ref_ptr<osg::Group> skyroot = new osg::Group;
    ...
    mSceneRootNode = new osg::Group;
    mRootNode = skyroot;

    skygroup->addChild(mSceneRootNode);
    skygroup->addChild(mRootNode);

    mRootNode->setNodeMask(Mask_Sky);
    parentNode->addChild(skygroup);
    ...
    mUnderwaterSwitch = new UnderwaterSwitchCallback(skygroup);
}
...
void SkyManager::setWeather(const WeatherResult& weather)
{
    ...
    if (mCurrentParticleEffect != weather.mParticleEffect)
    {
        ...
        if (mCurrentParticleEffect.empty())
        {
            if (mParticleNode)
            {
                mSceneRootNode->removeChild(mParticleNode);
