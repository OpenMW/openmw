...
void NpcAnimation::updateParts()
{
    ...
    for(size_t i = 0;i < slotlistsize && mViewMode != VM_HeadOnly;i++)
    {
        ...
        if(store->getType() == typeid(ESM::Clothing).name())
        {
        ...
        else if(store->getType() == typeid(ESM::Armor).name())
        {
    ...
    if(mPartPriorities[ESM::PRT_Shield] < 1)
    {
        ...
        if(store != inv.end() && (part=*store).getType() == typeid(ESM::Light).name())
        {
...
void NpcAnimation::showWeapons(bool showWeapon)
{
    ...
    if(showWeapon)
    {
        ...
        if(weapon != inv.end())
        {
            ...
            if (weapon->getType() == typeid(ESM::Weapon).name() &&
...
void NpcAnimation::showCarriedLeft(bool show)
{
    ...
    if(show && iter != inv.end())
    {
        ...
        if (iter->getType() == typeid(ESM::Armor).name())
        {
                                        ...
                                        mesh, !iter->getClass().getEnchantment(*iter).empty(), &glowColor))
        {
            ...
            if (iter->getType() == typeid(ESM::Light).name() && mObjectParts[ESM::PRT_Shield])
