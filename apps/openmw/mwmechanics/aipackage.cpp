...
namespace MWMechanics
{
    ...
    MWWorld::Ptr AiPackage::getTarget() const
    {
        return MWWorld::Ptr();
    }

    bool AiPackage::sideWithTarget() const
    {
        return false;
    }

    bool AiPackage::followTargetThroughDoors() const
    {
        return false;
    }

    bool AiPackage::canCancel() const
    {
        return true;
    }

    bool AiPackage::shouldCancelPreviousAi() const
    {
        return true;
    }

    bool AiPackage::getRepeat() const
    {
        return false;
    }

    AiPackage::AiPackage() : mTimer(0.26f), mStarted(false) { //mTimer starts at .26 to force initial pathbuild
    }

    bool AiPackage::pathTo(const MWWorld::Ptr& actor, ESM::Pathgrid::Point dest, float duration)
    {
    ...
    void AiPackage::evadeObstacles(const MWWorld::Ptr& actor, float duration, const ESM::Position& pos)
    {
    ...
    bool AiPackage::doesPathNeedRecalc(ESM::Pathgrid::Point dest, const ESM::Cell *cell)
    {
        return distance(mPrevDest, dest) > 10;
    }

    bool AiPackage::isTargetMagicallyHidden(const MWWorld::Ptr& target)
    {
    ...
    bool AiPackage::isNearInactiveCell(const ESM::Position& actorPos)
    {
