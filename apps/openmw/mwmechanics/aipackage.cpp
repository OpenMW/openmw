...
{
    const MagicEffects& magicEffects(target.getClass().getMagicEffects(target));
    return (magicEffects.magnitude(ESM::MagicEffect::Invisibility) > 0)
        || (magicEffects.magnitude(ESM::MagicEffect::Chameleon) > 75);
}
