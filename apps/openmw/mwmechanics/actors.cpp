                ...
                if (!iter->first.getClass().getCreatureStats(iter->first).isDead())
                {
                    ...
                    if (MWBase::Environment::get().getMechanicsManager()->isAIActive() && inProcessingRange)
                    {
                        ...
                        if (timerUpdateHeadTrack == 0)
                        {
                            ...
                            if (!((iter->first == player) && MWBase::Environment::get().getWorld()->getPlayer().getThirdPersonOverShoulderRanged()) && !iter->first.getClass().getCreatureStats(iter->first).getKnockedDown())                            
                            {
                                for (PtrActorMap::iterator it(mActors.begin()); it != mActors.end(); ++it)
                                {
