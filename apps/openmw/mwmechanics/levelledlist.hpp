...
namespace MWMechanics
{
    ...
    inline std::string getLevelledItem (const ESM::LevelledListBase* levItem, bool creature, Misc::Rng& generator = Misc::Rng::sInstance)
    {
        ...
        if (generator.exclusiveRandom(100) < levItem->mChanceNone)
            return std::string();
        ...
        if (candidates.empty())
            return std::string();
        std::string item = candidates[generator.exclusiveRandom(candidates.size())];
        ...
        {
            if (ref.getPtr().getTypeName() == typeid(ESM::ItemLevList).name())
                return getLevelledItem(ref.getPtr().get<ESM::ItemLevList>()->mBase, false, generator);
            else
                return getLevelledItem(ref.getPtr().get<ESM::CreatureLevList>()->mBase, true, generator);
        }
    }
