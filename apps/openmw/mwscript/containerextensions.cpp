...
namespace
{
    ...
    {
        if(itemPtr.getType() == typeid(ESM::ItemLevList).name())
        {
        ...
        class OpAddItem : public Interpreter::Opcode0
        {
            public:
                ...
                void execute (Interpreter::Runtime& runtime) override
                {
                    ...
                    bool isLevelledList = itemPtr.getClass().getType() == typeid(ESM::ItemLevList).name();
                    if(!isLevelledList)
                    ...
                    if(ptr.getClass().getType() == typeid(ESM::Container).name() && (!ptr.getRefData().getCustomData() ||
        ...
        class OpRemoveItem : public Interpreter::Opcode0
        {
            public:
                ...
                void execute (Interpreter::Runtime& runtime) override
                {
                    ...
                    else if(ptr.getClass().getType() == typeid(ESM::Container).name() &&
        ...
        class OpGetArmorType : public Interpreter::Opcode0
        {
            public:
                ...
                void execute(Interpreter::Runtime &runtime) override
                {
                    ...
                    if (it == invStore.end() || it->getType () != typeid(ESM::Armor).name())
                    {
                        runtime.push(-1);
                        return;
                    }
        ...
        class OpGetWeaponType : public Interpreter::Opcode0
        {
            public:
                ...
                void execute(Interpreter::Runtime &runtime) override
                {
                    ...
                    else if (it->getType() != typeid(ESM::Weapon).name())
                    {
                        if (it->getType() == typeid(ESM::Lockpick).name())
                        {
                            runtime.push(-2);
                        }
                        else if (it->getType() == typeid(ESM::Probe).name())
                        {
                            runtime.push(-3);
                        }
                        else
