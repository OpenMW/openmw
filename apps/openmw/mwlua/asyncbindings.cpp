...
namespace MWLua
{
    ...
    sol::function getAsyncPackageInitializer(const Context& context)
    {
        ...
        api["registerTimerCallback"]
            = [](const AsyncPackageId& asyncId, std::string_view name, sol::function callback) {
        ...
        api["newUnsavableSimulationTimer"]
            = [world = context.mWorldView](const AsyncPackageId& asyncId, double delay, sol::function callback) {
                  asyncId.mContainer->setupUnsavableTimer(TimerType::SIMULATION_TIME,
                      world->getSimulationTime() + delay, asyncId.mScriptId, std::move(callback));
              };
