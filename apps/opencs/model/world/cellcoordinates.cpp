...
CSMWorld::CellCoordinates::CellCoordinates(int x, int y)
    : mX(x)
    , mY(y)
{
}
...
{
    return static_cast<int>(vertexGlobal
        - std::floor(static_cast<float>(vertexGlobal) / (ESM::Land::LAND_SIZE - 1)) * (ESM::Land::LAND_SIZE - 1));
}
