  ...
  mIpcServerName ("org.openmw.OpenCS"), mServer(NULL), mClientSocket(NULL)
{
    ...
    connect (&mFileDialog, SIGNAL(signalOpenFiles (const sfs::path&)),
             this, SLOT(openFiles (const sfs::path&)));

    connect (&mFileDialog, SIGNAL(signalCreateNewFile (const sfs::path&)),
             this, SLOT(createNewFile (const sfs::path&)));
    connect (&mFileDialog, SIGNAL (rejected()), this, SLOT (cancelFileDialog ()));

    connect (&mNewGame, SIGNAL (createRequest (const sfs::path&)),
             this, SLOT (createNewGame (const sfs::path&)));
...
void CS::Editor::openFiles (const sfs::path &savePath)
{
...
void CS::Editor::createNewFile (const sfs::path &savePath)
{
    std::vector<sfs::path> files;
...
void CS::Editor::createNewGame (const sfs::path& file)
{
    std::vector<sfs::path> files;
...
bool CS::Editor::makeIPCServer()
{
    try
    {
        mPid = sfs::temp_directory_path();
        mPid /= "openmw-cs.pid";
        bool pidExists = sfs::exists(mPid);
        ...
        if(pidExists)
        {
            ...
            if(sfs::exists(fullPath.toUtf8().constData()))
            {
