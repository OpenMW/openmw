...
namespace ESSImport
{
    ...
    void convertPCDT(const PCDT& pcdt, ESM::Player& out, std::vector<std::string>& outDialogueTopics, bool& firstPersonCam, bool& teleportingEnabled, bool& levitationEnabled, ESM::ControlsState& controls)
    {
        ...
        for (const auto & mFaction : pcdt.mFactions)
        {
            ESM::NpcStats::Faction faction;
            faction.mExpelled = (mFaction.mFlags & 0x2) != 0;
            faction.mRank = mFaction.mRank;
            faction.mReputation = mFaction.mReputation;
            out.mObject.mNpcStats.mFactions[Misc::StringUtils::lowerCase(mFaction.mFactionName.toString())] = faction;
        }
        for (int i=0; i<3; ++i)
        ...
        for (const auto & mKnownDialogueTopic : pcdt.mKnownDialogueTopics)
        {
            outDialogueTopics.push_back(Misc::StringUtils::lowerCase(mKnownDialogueTopic));
        }
